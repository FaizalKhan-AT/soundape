...
const Sidebar: FC = () => {
  ...
  return (
    <>
      <Modal title="Search" modalRef={modalRef} id="" />
      <nav
      ...
      >
        ...
        <ul className="d-flex flex-column px-2 my-3 gap-4 menu-container">
          {menuItems.map((item, idx) => {
            if (item.name === "Profile" && authState.isLoggedIn === false)
              return;
            return (
              <li
                key={item.name + idx}
        ...
        {authState.isLoggedIn ? (
          ...
          >
            <Link to="/signin">Sign In</Link> to react, create and grow. New
            here <Link to="/signup">create an account</Link>
          </div>
        )}
      </nav>
    </>
